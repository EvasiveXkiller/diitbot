...
client.on("message", (message) => {
	...
	if (command === "timetable") {
		...
		if (userinput === "" || userinput === "help") {
			...
			if (daytomorrow === "saturday" || daytomorrow === "sunday") {
				...
				const preprocess = Object.entries(
	...
	if (command === "replacements") {
		const x = dbjack.get("reminder").value(); // refers to the reminder array
		for (let i = 0; i < x.length; i++) {
			...
			const differenceobject = difference.toObject(); // this is needed to change it to an object
			const subjecttitle = dbjack.get("reminder").value()[i].event; // subjecttitle refers to the event section in the reminder array in database
			const embed = new Discord.MessageEmbed({
	...
	if (command === "eventinsert") {
		dbjack.read();
		const userinputpre = message.content.replace("$eventinsert", "").trim();
		const userinputSplit = userinputpre.split(",");
		console.log(userinputSplit);

		// * checking begins here
		const checkDate = DateTime.fromISO(userinputSplit[1]);
		if (!checkDate.isValid) {
			const dateinvalidembed = new Discord.MessageEmbed({
				title: "Date Invalid",
		...
		const comfirmembed = new Discord.MessageEmbed({
			title: "Comfirm?",
			description:
				"Event : "
				+ userinputSplit[0]
